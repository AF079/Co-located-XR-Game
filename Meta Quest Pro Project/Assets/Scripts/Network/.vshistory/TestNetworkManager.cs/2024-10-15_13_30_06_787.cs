...
public class TestNetworkManager : MonoBehaviourPunCallbacks
{
    ...
    async Task<int> LoadAnchorByUuid()
    {
        // Step 1: Load
        IEnumerable<Guid> uuids = new List<Guid>(guidList);
        ...
        if (result.Success)
        {
            ...
            Debug.Log("Number of anchors in buffer: " + result.Value.Count); 
            ...
            var result2 = await OVRSpatialAnchor.EraseAnchorsAsync(spatialAnchors, null);
            if (result.Success)
            {
            ...
            {
                Debug.LogError($"Failed to erase anchors {spatialAnchors.Count} with result {result.Status}");
            }
            return 0;
        }
        else

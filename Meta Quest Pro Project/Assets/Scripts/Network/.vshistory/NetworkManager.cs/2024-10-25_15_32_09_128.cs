...
*/
    ...
    private void move(Transform t)
    {
        ...
        Vector3 v_ = new Vector3(r_originBinA.x,r_upBinA.y, r_originBinA.z) - r_originBinA; // also try r_originBinA.z instead of r_upBinA.z
        ...
        Vector3 translatedVector = input + translation;  // (x', y', z')

        // Step 2: Compute vectors v and v'
        Vector3 v = v1 - translation;  // (p, q, r) - (a, b, c)
        Vector3 vPrime = v2 - translation;  // (a, q, r) - (a, b, c)

        // Step 3: Compute the rotation quaternion between v and v'
        Quaternion rotation = Quaternion.FromToRotation(vPrime, v);

        // Step 4: Rotate the translated vector
        Vector3 rotatedVector = rotation * translatedVector;

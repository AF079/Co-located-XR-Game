...
*/
    ...
    private void repositionSpheres()
    {
        ...
        if (spheres.Length == numberOfBalls) {
            ...
            Vector3 newPos = ovrCameraRig.centerEyeAnchor.position;
            newPos += translation;
            ovrCameraRig.centerEyeAnchor.position = new Vector3(newPos.x, newPos.y, newPos.z);
            Vector3 dirA = rp2 - rp1;
            Vector3 dirB = p2 - p1;
            Quaternion rotation = Quaternion.FromToRotation(dirB, dirA);
            ovrCameraRig.centerEyeAnchor.rotation *= rotation;

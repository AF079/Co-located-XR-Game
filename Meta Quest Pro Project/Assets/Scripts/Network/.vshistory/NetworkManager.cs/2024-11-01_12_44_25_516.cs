...
public class NetworkManager : MonoBehaviourPunCallbacks
{
    ...
    /*    private void move(Transform t)
        {
            ...
            Vector3 newPos = MatMul(transormationMatrix, t.position);
            t.position = new Vector3(newPos.x, newPos.y, newPos.z);
        }*/
    ...
    private void move(Transform t)
    {
        ...
        // Step 2: Create the rotation matrix for B’s coordinate system in A’s space
        float[,] rotationMatrix = { { rightB.x, upB.x, forwardB.x },
                                { rightB.y, upB.y, forwardB.y },
                                { rightB.z, upB.z, forwardB.z } };

        // Step 3: Translate t.position relative to B’s origin in A’s space
        Vector3 localPosition = t.position + originB;

        // Step 4: Apply the rotation to align with B’s axes
        Vector3 rotatedPosition = MatMul(rotationMatrix, localPosition);

        // Step 5: Translate back to B’s origin to complete the transformation
        Vector3 alignedPosition = rotatedPosition - originB;

        // Step 6: Update the transform position in B’s aligned space
        t.position = new Vector3(alignedPosition.x, alignedPosition.y, alignedPosition.z);
    }

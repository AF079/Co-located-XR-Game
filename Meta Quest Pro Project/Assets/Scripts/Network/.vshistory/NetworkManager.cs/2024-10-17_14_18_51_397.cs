...
/*    private IEnumerator EraseCoroutine(OVRSpatialAnchor anchor)
    ...
    private IEnumerator spawnBalls()
    {
        for (var i = 0; i < numberOfBalls; i++)
        {
            ...
            //Instantiate(pAnchor,randomPosition,Quaternion.identity);
            ...
            //Debug.Log("ANCHOR CREATED STATE: " + anchor.Created + " ANCHOR LOCALIZED STATE: " + anchor.Localized);
...
/*        Debug.Log("SAVING ANCHORS");
        Task task = SaveAnchors(spatialAnchors);
        yield return new WaitUntil(() => task.IsCompleted);

        Debug.Log("SHARING ANCHORS");
        foreach (OVRSpatialAnchor anchor in spatialAnchors)
        {
            task = ShareAnchor(anchor);
            yield return new WaitUntil(() => task.IsCompleted);
            Debug.Log("BROADCASTING UUID: " + anchor.Uuid);
            photonView_.RPC("BroadcastUuidToOtherUser", RpcTarget.Others, anchor.Uuid.ToString());

        }*/

    }
/*    public async Task<int> SaveAnchors(List<OVRSpatialAnchor> anchors)
    {
        ...
        {
            Debug.LogError($"ANCHORS FAILED TO SAVE WITH ERROR {result.Status}");
            return 1;
        }
    }
    public async Task<int> ShareAnchor(OVRSpatialAnchor anchor)
    {
        Debug.Log("SHARING ANCHOR: " + anchor.Uuid.ToString() + " with " + otherUser.Id.ToString());
    ...
    private IEnumerator TryLoadAnchors()
    {
        ...
        Debug.Log("FINISHED WAITING BEFORE LOADING");
    ...
    {
        Debug.Log("I HAVE " + guidList.Count + " GUIDS TRYING TO LOAD ANCHORS");
        foreach(var guid in guidList)
        {
            Debug.Log("LOADING GUID: " + guid);
        }
        var result = await OVRSpatialAnchor.LoadUnboundSharedAnchorsAsync(guidList, _unboundAnchors);
        Debug.Log("RESULT: " + result.Status.ToString() + " " + result.Success + " " + _unboundAnchors.Count);
        if (result.Success)
        {
            ...
            foreach(var unboundAnchor in result.Value) {
                unboundAnchor.LocalizeAsync().ContinueWith((success,anchor)=> {
                    if (success)
                    {
                        Debug.Log("ADDING TO RECEIVED ANCHORS");
    ...
    private void BroadcastUuidToOtherUser(string uuid_str)
    {
        ...
        Debug.Log("ADDING TO GUID LIST " + uuid_str + " " + uuid);
        guidList.Add(uuid);
    }
    [PunRPC]
...
/*        if (!PhotonNetwork.IsMasterClient)
        {
            ...
            else if(recievedAnchors.Count == numberOfBalls && guidList.Count == numberOfBalls)
            {

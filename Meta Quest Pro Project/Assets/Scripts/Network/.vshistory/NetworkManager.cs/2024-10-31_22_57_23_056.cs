...
public class NetworkManager : MonoBehaviourPunCallbacks
{
    ...
    private void move(Transform t)
    {
        /*// Step 1: Define the basis vectors for B’s coordinate system in A’s space
        Vector3 originB = r_originBinA;
        Vector3 upB = (r_upBinA - r_originBinA).normalized;           // Up vector for B in A’s space
        Vector3 forwardB = (r_forwardBinA - r_originBinA).normalized; // Forward vector for B in A’s space
        Vector3 rightB = (r_sideBinA - r_originBinA).normalized;//Vector3.Cross(upB, forwardB).normalized;     // Right vector, orthogonal to up and forward

        // Step 2: Build the transformation matrix using B's coordinate system in A’s space
        Matrix4x4 transformMatrix = new Matrix4x4();
        transformMatrix.SetColumn(0, new Vector4(rightB.x, rightB.y, rightB.z, 0));    // X-axis (right)
        transformMatrix.SetColumn(1, new Vector4(upB.x, upB.y, upB.z, 0));             // Y-axis (up)
        transformMatrix.SetColumn(2, new Vector4(forwardB.x, forwardB.y, forwardB.z, 0)); // Z-axis (forward)
        transformMatrix.SetColumn(3, new Vector4(originB.x, originB.y, originB.z, 1)); // Origin position

        // Step 3: Translate and rotate t’s position from A’s space to B’s space
        Vector3 localPositionInA = t.position - r_originBinA;
        Vector3 alignedPositionInB = transformMatrix.MultiplyPoint3x4(localPositionInA);

        // Step 4: Update the transform position to the new aligned position in B’s space
        t.position = new Vector3(alignedPositionInB.x, alignedPositionInB.y, alignedPositionInB.z);*/

        // Translate position to be relative to B’s origin in A’s space
        Vector3 localPosition = t.position - r_originBinA;

        // Define all three basis vectors for B in A’s space
        Vector3 upB = (r_upBinA - r_originBinA).normalized;
        Vector3 forwardB = (r_forwardBinA - r_originBinA).normalized;
        Vector3 rightB = Vector3.Cross(upB, forwardB).normalized;

        // Align each axis in sequence, using Quaternion.FromToRotation
        Quaternion rotationToAlign = Quaternion.FromToRotation(Vector3.up, upB);
        localPosition = rotationToAlign * localPosition;

        rotationToAlign = Quaternion.FromToRotation(rotationToAlign * Vector3.forward, forwardB) * rotationToAlign;
        localPosition = rotationToAlign * localPosition;

        rotationToAlign = Quaternion.FromToRotation(rotationToAlign * Vector3.right, rightB) * rotationToAlign;
        localPosition = rotationToAlign * localPosition;

        Vector3 res = localPosition + r_originBinA;
        // Apply translation back to B’s origin
        t.position = new Vector3(res.x,res.y, res.z);

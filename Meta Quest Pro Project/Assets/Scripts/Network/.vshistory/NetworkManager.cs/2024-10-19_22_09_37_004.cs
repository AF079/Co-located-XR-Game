...
public class NetworkManager : MonoBehaviourPunCallbacks
{
    ...
    private float TrilinearInterpolation(float u, float v, float w, float pMax, float pMin, float[] points)
    {
        float a = 1 - u;
        float b = 1 - v;
        float c = 1 - w;
    ...
    private Vector3 getInterpolatedPoint(Vector3 pointToInterpolate)
    {
        ...
        float newX = TrilinearInterpolation(u, v, w, p000.x,p111.x,X);
        float newY = TrilinearInterpolation(u, v, w, p000.x, p111.x, Y);
        float newZ = TrilinearInterpolation(u, v, w, p000.x, p111.x, Z);

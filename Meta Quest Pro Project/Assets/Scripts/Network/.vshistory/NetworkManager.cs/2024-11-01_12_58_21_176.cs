...
public class NetworkManager : MonoBehaviourPunCallbacks
{
    ...
    private void move(Transform t,bool isSphere)
    {
        Vector3 originB = r_originBinA;
        Vector3 upB = (r_upBinA - r_originBinA).normalized;
        Vector3 forwardB = (r_forwardBinA - r_originBinA).normalized;
        ...
        Vector3 localPosition = t.position;
        if (isSphere)
        {
            localPosition = t.position - originB;
    ...
    private IEnumerator repositionSpheres()
    {
        ...
        if (spheres.Length == numberOfBalls) {
            ...
            move(origin.transform,false);
            move(up.transform, false);
            move(side.transform, false);
            move(forward.transform, false);
            move(canvas1.transform, false);
            move(canvas2.transform, false);
            foreach (GameObject sphere in spheres)
            {

                //Debug.Log("OLD POSITION: " + sphere.transform.position);
                move(sphere.transform,true);

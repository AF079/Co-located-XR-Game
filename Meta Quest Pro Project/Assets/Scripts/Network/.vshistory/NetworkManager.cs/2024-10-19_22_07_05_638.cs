...
public class NetworkManager : MonoBehaviourPunCallbacks
{
    ...
    private float TrilinearInterpolation(float p, float u, float v, float w, float pMax, float pMin, float[] points)
    {
        float a = 1 - u;
        float b = 1 - v;
        float c = 1 - w;
    ...
    {
        Vector3 p000 = new Vector3(Mathf.Min(rp1.x, rp2.x), Mathf.Min(rp1.y, rp2.y), Mathf.Min(rp1.z, rp2.z));
        Vector3 p111 = new Vector3(Mathf.Max(rp1.x, rp2.x), Mathf.Max(rp1.y, rp2.y), Mathf.Max(rp1.z, rp2.z));
        ...
        float u = normalize(x, p000.x, p111.x);
        float v = normalize(y, p000.y, p111.y);
        float w = normalize(z, p000.z, p111.z);
        
        float newX = TrilinearInterpolation(x,u, v,w,p000.x,p111.x,X);
        float newY = TrilinearInterpolation(y, u, v, w, p000.x, p111.x, Y);
        float newZ = TrilinearInterpolation(z, u, v, w, p000.x, p111.x, Z);
    ...
    private void sendTransformData(float x2, float y2, float z2, float x1, float y1, float z1)
    {
        ...
        /*        rec_p12 = new Vector3(x12, y12, z12);
                ...
                LeftEye.transform.position += translation;
                CenterEye.transform.position += translation;
                RightEye.transform.position += translation;

                LeftHand.transform.position += translation;
                RightHand.transform.position += translation;


                Quaternion rotation = Quaternion.FromToRotation(p12, rec_p12);

                LeftEye.transform.rotation *= rotation;
                CenterEye.transform.rotation *= rotation;

...
public class NetworkManager : MonoBehaviourPunCallbacks
{
...

    {
        ...
        /*       
                ...
                Vector3 axisZ = Vector3.Cross(forward, forwardB).normalized;
                float angleZ = Mathf.Acos(Vector3.Dot(forward, forwardB));
                Quaternion qz = Quaternion.AngleAxis(angleZ,axisZ);

                Vector3 rotatedX = qz * right;
                Vector3 axisX = Vector3.Cross(rotatedX, rightB).normalized;
                float angleX = Mathf.Acos(Vector3.Dot(rotatedX,rightB));
                Quaternion qx = Quaternion.AngleAxis(angleX,axisX);

                Quaternion q = qx * qz;

                Vector3 newPos = q * t.position;
        ...
        Vector3 alignedPosition = rotatedPosition + originB;//+ originB;

...
*/
    ...
    private void repositionSpheres()
    {
        ...
        if (spheres.Length == numberOfBalls) {
            ...
            Debug.Log("BEFORE CENTER EYE " + overCameraRig.centerEyeAnchor.position + " " + overCameraRig.centerEyeAnchor.rotation);


            Vector3 translation = rp1 - p1;
            Vector3 newPos = overCameraRig.centerEyeAnchor.position;
            newPos += translation;
            overCameraRig.transform.position = new Vector3(newPos.x, newPos.y, newPos.z);
            Vector3 dirA = rp2 - rp1;
            Vector3 dirB = p2 - p1;
            Quaternion rotation = Quaternion.FromToRotation(dirB, dirA);
            overCameraRig.centerEyeAnchor.rotation *= rotation;

            Debug.Log("AFTER CENTER EYE " + overCameraRig.centerEyeAnchor.position + " " + overCameraRig.centerEyeAnchor.rotation);

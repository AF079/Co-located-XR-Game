...
public class NetworkManager : MonoBehaviourPunCallbacks
{
    ...
    private IEnumerator StartPlayerAlign()
    {
        ...
        //Player targetPlayer = PhotonNetwork.CurrentRoom.GetPlayer(clientActorNumber);
        GameObject client = GameObject.Find("ClientManager");
        while (client == null) yield return null;
        PhotonView client_pv = client.GetComponent<PhotonView>();


        if (client_pv != null)
        {
            Debug.Log("CLIENT NUM: " + clientActorNumber);
            Debug.Log("CALLING RPC");

            client_pv.RPC("receiveTransformData", RpcTarget.AllBuffered,
        ...
        {
            Debug.LogWarning("Player with Actor Number " + clientActorNumber + " not found.");
        }*/

...
*/
    ...
    private void repositionSpheres()
    {
        ...
        if (spheres.Length == numberOfBalls) {
            ...
            Vector3 dirB = p2 - p1;
            Quaternion rot = Quaternion.FromToRotation(dirB, dirA);
            foreach(GameObject sphere in spheres)
            {
                ...
                /*sphere.transform.position = rot * (sphere.transform.position - p1) + rp1;*/

                sphere.transform.position -= p1;

                // Step 2: Apply rotation
                sphere.transform.position = rot * sphere.transform.position;

                // Step 3: Translate to the new position
                sphere.transform.position += rp1;
    ...
    void Update()
    {
        ...
        if(!hasRepositioned && !PhotonNetwork.IsMasterClient && rp1 != Vector3.zero &&  rp2 != Vector3.zero)
        {
            repositionSpheres();
        }

...
public class NetworkManager : MonoBehaviourPunCallbacks
{
    ...
    private void move(Transform t)
    {
        ...
        //t.position = newPos*rotation;
    }
    private void repositionSpheress()
    {
        GameObject[] spheres = GameObject.FindGameObjectsWithTag("tBall");
        if (spheres.Length == numberOfBalls)
        {
            // Create rotation from B's space to A's space
            Quaternion rot = Quaternion.FromToRotation(Vector3.zero, p1); // Modify rotation logic here depending on context

            foreach (GameObject sphere in spheres)
            {
                Debug.Log("OLD POSITION: " + sphere.transform.position);

                // Step 1: Translate to Player B's virtual world origin
                Vector3 newPos = sphere.transform.position - p1;

                // Step 2: Apply the rotation
                newPos = rot * newPos;

                // Step 3: Translate to Player A's physical world position
                newPos += rp1; // Add Player A's corresponding point
    ...
    private IEnumerator repositionSpheres()
    {
        ...
        if (spheres.Length == numberOfBalls) {
            ...
            foreach (GameObject sphere in spheres)
            {
                ...
                Debug.Log("Synchronization mode: " + photonView.Synchronization);

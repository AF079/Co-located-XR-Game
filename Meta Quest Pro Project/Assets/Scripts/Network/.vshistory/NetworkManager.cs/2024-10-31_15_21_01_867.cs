...
public class NetworkManager : MonoBehaviourPunCallbacks
{
    ...
    private void move(Transform t)
    {
        ...
        Vector3 v = (r_upBinA - r_originBinA).normalized;

        Vector3 v_ = Vector3.up;
        ...
        Quaternion rot = Quaternion.FromToRotation(v_, v);
        Vector3 newPos = rot * u + r_originBinA;
        
        t.position = new Vector3(newPos.x, newPos.y,newPos.z); 
        Debug.Log("NEWPOS? " + newPos + " " + u + " " + v + " " + v_ + " " + rot);*/
        /*        // Step 1: Calculate the basis vectors for B in A’s space
                Vector3 originB = r_originBinA;
                Vector3 upB = (r_upBinA - r_originBinA).normalized;

                // Assuming 'rightB' can be a cross-product to maintain a perpendicular axis, 
                // we’ll use A’s right axis and cross it with upB.
                Vector3 forwardB = Vector3.Cross(Vector3.right, upB).normalized;
                Vector3 rightB = Vector3.Cross(upB, forwardB).normalized;

                // Step 2: Create the transformation matrix for B’s space in A’s coordinate system
                Matrix4x4 transformMatrix = new Matrix4x4();
                transformMatrix.SetColumn(0, rightB);    // X-axis of B in A's space
                transformMatrix.SetColumn(1, upB);       // Y-axis of B in A's space
                transformMatrix.SetColumn(2, forwardB);  // Z-axis of B in A's space
                transformMatrix.SetColumn(3, new Vector4(originB.x, originB.y, originB.z, 1)); // Origin

                // Step 3: Translate and rotate point in A’s space to align with B’s space
                Vector3 pointInA = t.transform.position;
                Vector3 pointInB = transformMatrix.MultiplyPoint3x4(pointInA - r_originBinA);

                // Step 4: Set the new position in B’s space
                t.transform.position = new Vector3(pointInB.x, pointInB.y, pointInB.z);*/
    }

...
/*    private void move(Transform t)
    ...
    private void move(Transform t)
    {
        // Step 1: Translate to be relative to B's origin in A's space
        Vector3 u = t.position - r_originBinA;

        // Step 2: Compute the rotation to align A's "up" with B's "up"
        Vector3 vu = (r_upBinA - r_originBinA).normalized;   // B's up in A's space
        Quaternion rotationToAlign = Quaternion.FromToRotation(Vector3.up, vu);

        // Step 3: Apply rotation to the translated position

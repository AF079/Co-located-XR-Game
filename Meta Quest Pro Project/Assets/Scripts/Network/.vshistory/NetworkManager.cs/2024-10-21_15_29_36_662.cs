...
*/
    ...
    private void repositionSpheres()
    {
        ...
        if (spheres.Length == numberOfBalls) {
            ...
            Vector3 dirB = p2 - p1;
            Quaternion rot = Quaternion.FromToRotation(Vector3.zero, p1);
            foreach (GameObject sphere in spheres)
            {
                Debug.Log("OLD POSITION: " + sphere.transform.position);
                *//*sphere.transform.position += translation;
            ...
            Quaternion rot = Quaternion.FromToRotation(Vector3.forward, p1); // Adjust to the right rotation based on how p1 and Player A's origin align

            foreach (GameObject sphere in spheres)
            {
                Debug.Log("OLD POSITION: " + sphere.transform.position);

                // Step 1: Translate spheres to Player B's local origin
                Vector3 newPos = sphere.transform.position - p1;

                // Step 2: Rotate the sphere's position to align with Player A's orientation
                newPos = rot * newPos;

                // Step 3: Translate spheres to Player A's world
                newPos += rp1;

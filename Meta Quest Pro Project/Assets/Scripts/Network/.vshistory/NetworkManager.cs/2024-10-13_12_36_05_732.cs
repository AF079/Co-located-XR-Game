...
public class NetworkManager : MonoBehaviourPunCallbacks
{
    ...
    private IEnumerator StartTimer()
    {
        ...
        {
            Debug.Log("Conditions not met, waiting to retry...");

            // Wait for 1 second before checking the condition again
            yield return new WaitForSeconds(1f);
        }

        // When the condition is true, wait 5 seconds before proceeding
        yield return new WaitForSeconds(5);

        Debug.Log("5 seconds passed, conditions met!");
        spawnBalls();  // Call spawnBalls() once the conditions are true
    }
    [PunRPC]

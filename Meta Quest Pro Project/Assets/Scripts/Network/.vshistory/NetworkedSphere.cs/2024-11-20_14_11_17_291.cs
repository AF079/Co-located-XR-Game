    ...
    void FixedUpdate()
    {
        ...
        if (PhotonNetwork.IsMasterClient)
        {
            ...
            if (distance > 0.01f) // Ignore small movements
            {
                ...
                rb.position = Vector3.MoveTowards(transform.position, simulatedPosition, Time.deltaTime * 0.5f);
            }
        ...
        else
        {
            ...
            if (distance > 0.01f)
            {
                rb.position = Vector3.MoveTowards(transform.position, targetPosition, Time.deltaTime * 0.5f);
            }
        }
        /*if (!PhotonNetwork.IsMasterClient)
        {

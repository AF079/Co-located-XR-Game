...
public class NetworkedSphere : MonoBehaviour, IPunOwnershipCallbacks, IPunObservable
{
    ...
    void FixedUpdate()
    {
        ...
        if (PhotonNetwork.IsMasterClient)
        {
            ...
            rb.MovePosition(Vector3.MoveTowards(rb.position, simulatedPosition, Time.fixedDeltaTime * 5f));
        ...
        else
        {
            ...
            {
                rb.MovePosition(Vector3.MoveTowards(rb.position, targetPosition, Time.fixedDeltaTime * 5f));
            }
            else
            {

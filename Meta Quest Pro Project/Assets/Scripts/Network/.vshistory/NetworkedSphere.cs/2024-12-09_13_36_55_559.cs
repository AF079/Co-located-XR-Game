...
public class NetworkedSphere : MonoBehaviour, IPunOwnershipCallbacks, IPunObservable
{
    ...
    private float baseLatency = 0.01f;
    ...
    void FixedUpdate()
    {
        ...
        if (PhotonNetwork.IsMasterClient)
        {
            ...
            rb.MovePosition(Vector3.Lerp(rb.position, simulatedPosition, Time.timeScale * Time.fixedDeltaTime * 5f));
        ...
        else
        {
            ...
            {
                rb.MovePosition(Vector3.Lerp(rb.position, targetPosition, Time.timeScale* Time.fixedDeltaTime * 5f));
            }
            else
            {

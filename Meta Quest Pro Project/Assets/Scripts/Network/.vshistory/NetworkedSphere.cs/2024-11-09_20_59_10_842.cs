...
public class NetworkedSphere : MonoBehaviour, IPunOwnershipCallbacks, IPunObservable
{
    ...
    public void OnPhotonSerializeView(PhotonStream stream, PhotonMessageInfo info)
    {
        ...
        else if(!PhotonNetwork.IsMasterClient && stream.IsReading) //stream.Count > 0 && 
        {
            ...
            try
            {
                ...
                Vector3 localPosition = targetPosition - NetworkManager_test.r_originBinA;
                Vector3 rotatedPosition = NetworkManager_test.MatMul(NetworkManager_test.transformationMatrix, localPosition);

                /*Vector3 pos = NetworkManager_test.move(targetPosition);*/

                targetPosition = new Vector3(rotatedPosition.x, rotatedPosition.y, rotatedPosition.z);

...
public class NetworkedSphere : MonoBehaviour, IPunOwnershipCallbacks, IPunObservable
{
    ...
    void Update()
    {
        ...
        if (PhotonNetwork.IsMasterClient)
        {
            ...
            rb.position = Vector3.SmoothDamp(transform.position, simulatedPosition, ref velocity, 0.3f);

        }
        else
        {
            //transform.position = Vector3.Lerp(transform.position, targetPosition, Time.deltaTime);
            rb.position = Vector3.SmoothDamp(transform.position, targetPosition, ref velocity, 0.3f);
        }
    ...
    public void OnPhotonSerializeView(PhotonStream stream, PhotonMessageInfo info)
    {
        ...
        if (PhotonNetwork.IsMasterClient)
        {
...

            {
                ...
                Vector3 localPosition = transform.position - NetworkManager.originBinA;

...
public class NetworkedSphere : MonoBehaviour, IPunOwnershipCallbacks, IPunObservable
{
    ...
    void Update()
    {
        ...
        if (PhotonNetwork.IsMasterClient)
        {
            ...
            rb.position = Vector3.MoveTowards(rb.position, simulatedPosition, Time.deltaTime);//Vector3.SmoothDamp(rb.position, simulatedPosition, ref masterVelocity, 0.3f);

        }
        else
        {
            //transform.position = Vector3.Lerp(transform.position, targetPosition, Time.deltaTime);
            rb.position = Vector3.MoveTowards(rb.position, targetPosition, Time.deltaTime); //Vector3.SmoothDamp(rb.position, targetPosition, ref nonMasterVelocity, 0.3f);
        }

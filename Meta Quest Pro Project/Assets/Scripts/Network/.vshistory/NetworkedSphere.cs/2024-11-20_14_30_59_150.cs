    ...
    void FixedUpdate()
    {
        ...
        if (PhotonNetwork.IsMasterClient)
        {
            ...
            if (distance > 0.01f) // Ignore small movements
            {
                ...
                transform.position = Vector3.Lerp(rb.position, simulatedPosition, Time.deltaTime * 0.5f);
            }
            else
            {
        ...
        else
        {
            ...
            if (distance > 0.01f)
            {
                transform.position = Vector3.Lerp(rb.position, targetPosition, Time.deltaTime * 0.5f);
            }
            else
            {
                //rb.velocity = Vector3.zero;
    ...
    private IEnumerator NetworkSimulationCoroutine()
    {
        while (true)
        {
            ...
            if (UnityEngine.Random.value > packetLossProbability) 
            {
                ...
                velocityQueue.Enqueue(transform.velocity);
            }

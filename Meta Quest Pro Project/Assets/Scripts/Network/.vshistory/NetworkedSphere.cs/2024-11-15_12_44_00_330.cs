...
public class NetworkedSphere : MonoBehaviour, IPunOwnershipCallbacks, IPunObservable
{
    ...
    void Update()
    {
        ...
        handTimeStamp = Time.deltaTime;//PhotonNetwork.Time;
        prevLeftHandPos = LeftHand.transform.position;
        prevRightHandPos = RightHand.transform.position;
        ...
        {
            double curTime = Time.deltaTime;
            double deltaTime = curTime - handTimeStamp;
            if (isTouchingRight && RightHand != null)
            {
                Vector3 currentHandPosition = RightHand.transform.position;
                Vector3 handMovement = (currentHandPosition - prevRightHandPos) / (float)deltaTime;
                rb.AddForce(handMovement * pushForceMutliplier, ForceMode.Impulse);

            }
            else if (isTouchingLeft && LeftHand != null)
            {
                Vector3 currentHandPosition = LeftHand.transform.position;
                Vector3 handMovement = (currentHandPosition - prevLeftHandPos) / (float)deltaTime;
                rb.AddForce(handMovement * pushForceMutliplier, ForceMode.Impulse);

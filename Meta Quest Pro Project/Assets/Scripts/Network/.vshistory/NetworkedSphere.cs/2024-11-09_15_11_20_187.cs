...
public class NetworkedSphere : MonoBehaviour, IPunOwnershipCallbacks, IPunObservable
{
    ...
    private bool gotNewPos = false;
    private float delay = 0.5f;
    float sendTimer = 0f;
    private Vector3[] targetPositions = new Vector3[1000]; 
       ...
       /* if (Mathf.Abs(rb.transform.position.y) >= 5.0f || Mathf.Abs(rb.transform.position.x) >= 5.0f)
        ...
        if (!PhotonNetwork.IsMasterClient && photonView.IsMine)
        {
            if (curPosIdx == 0)
            {
                ...
                curPosIdx = (curPosIdx + 1) % 100;
            }
            else if(curPosIdx > 0)
            {
                if(transform.position == targetPositions[curPosIdx])
                {
                    curPosIdx = (curPosIdx + 1) % 100;
                }
                else
                {
                    transform.position = Vector3.MoveTowards(transform.position, targetPositions[curPosIdx], Time.deltaTime * lerpSpeed);
    ...
    public void OnPhotonSerializeView(PhotonStream stream, PhotonMessageInfo info)
    {
        ...
        if (PhotonNetwork.IsMasterClient)
        {
            ...
            if (stream.IsWriting && prevSentPos != transform.position && sendTimer >= delay)
            {

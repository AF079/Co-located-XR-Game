...
public class NetworkedSphere : MonoBehaviour, IPunOwnershipCallbacks, IPunObservable
{
    ...
    void Update()
    {
        ...
        if (PhotonNetwork.IsMasterClient)
        {
            ...
            Vector3 direction = (simulatedPosition - rb.position).normalized;
            float distance = Vector3.Distance(rb.position, simulatedPosition);

            if (distance > 0.01f) // Ignore small movements
            {
                rb.velocity = 0.1f * direction * distance / Time.fixedDeltaTime;
            }

...
public class NetworkedSphere : MonoBehaviour, IPunOwnershipCallbacks, IPunObservable
{
    ...
    void Update()
    {
        ...
        if (!PhotonNetwork.IsMasterClient && photonView.IsMine)
        {
            ...
            transform.position = Vector3.Lerp(transform.position, targetPosition.position, Time.deltaTime * lerpSpeed);
        }
    }
    /*
    ...
    public void OnPhotonSerializeView(PhotonStream stream, PhotonMessageInfo info)
    {
        ...
        if(PhotonNetwork.IsMasterClient && stream.IsWriting && prevSentPos != transform.position) {
        ...
        else if(stream.Count > 0 && !PhotonNetwork.IsMasterClient && stream.IsReading)
        {
            ...
            try
            {
                targetPosition.position = (Vector3)stream.ReceiveNext();

                Vector3 pos = NetworkManager_test.move(targetPosition);

                targetPosition.position = new Vector3(pos.x, pos.y, pos.z);

...
public class NetworkedSphere : MonoBehaviour, IPunOwnershipCallbacks, IPunObservable
{
    ...
    private float packetLossProbability = 100f;
    ...
    void Update()
    {
        ...
        if (PhotonNetwork.IsMasterClient)
        {
            float distance = Vector3.Distance(transform.position, simulatedPosition);

            if (distance > 0.01f) // Ignore small movements
            {
                
                transform.position = Vector3.MoveTowards(transform.position, simulatedPosition, Time.deltaTime * 0.5f);
            }

...
public class NetworkedSphere : MonoBehaviour, IPunOwnershipCallbacks, IPunObservable
{
    ...
    void FixedUpdate()
    {
        ...
        if (!PhotonNetwork.IsMasterClient && mag >= epsilon) //&& photonView.IsMine
        {
            rb.position = Vector3.MoveTowards(rb.position, targetPosition, Time.fixedDeltaTime);
    ...
    public void OnPhotonSerializeView(PhotonStream stream, PhotonMessageInfo info)
    {
        ...
        if (PhotonNetwork.IsMasterClient)
        {
            ...
            Debug.Log("Diff size " + mag);
            if (stream.IsWriting && mag >= epsilon)// && prevSentPos != transform.position
            {
    ...
    {
        if (!collision.collider.CompareTag("tBall"))
        {
            float x = Mathf.Sqrt(k_energy * 0.8f);//Mathf.Sqrt(k_energy);  * 0.8f
            rb.velocity *= x;

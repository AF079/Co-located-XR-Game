...
public class NetworkedSphere : MonoBehaviour, IPunOwnershipCallbacks, IPunObservable
{
    ...
    public void reqOwnershipForMove(Vector3 newPos)
    {
        this.newPos = newPos;
        shouldMove = true;
...
/*    public void OnPhotonSerializeView(PhotonStream stream, PhotonMessageInfo info)
    {
        if (stream.IsWriting)
        {
            // Sending position to other clients
            stream.SendNext(transform.position);
            stream.SendNext(rb.velocity);
        }
        else
        {
            // Receiving position from other clients
    ...
    public void move(Vector3 newPos)
    {
        if (photonView.IsMine)
        {
            ...
            if (rigidbodyView != null)
            {
                ...
                Debug.Log("DISABLED RBV");
            }

            if (transformView != null)
            {
                transformView.enabled = false;
                Debug.Log("DISABLED TV");
            }

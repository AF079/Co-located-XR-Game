...
public class NetworkedSphere : MonoBehaviour, IPunOwnershipCallbacks, IPunObservable
{
    ...
    void Start()
    {
        ...
        if (PhotonNetwork.IsMasterClient)
        {
            ...
            //sStartCoroutine(NetworkSimulationCoroutine());
        }
        else
        {
    ...
    void FixedUpdate()
    {
        if (!PhotonNetwork.IsMasterClient)
        {
            ...
            {
                rb.velocity = 0.012f * direction * distance / Time.fixedDeltaTime;
            }
            else
            {
        ...
        /*if (PhotonNetwork.IsMasterClient)
        {
            *//*Vector3 clampedPosition = new Vector3(rb.position.x, Mathf.Min(rb.position.y, newMaxHeight), rb.position.z);
            ...
            //Vector3.Lerp(rb.position, simulatedPosition, Time.fixedDeltaTime * 5f)
            rb.MovePosition(rb.position + direction * Time.fixedDeltaTime * 0.5f);
        ...
        else
        {
            *//*Vector3 clampedPosition = new Vector3(rb.position.x, Mathf.Min(rb.position.y, newMaxHeight), rb.position.z);
            ...
            float correctionThreshold = 0.01f; // 1 cm tolerance
            if (Vector3.Distance(rb.position, targetPosition) > correctionThreshold)
            {
                Vector3 direction = (targetPosition - rb.position).normalized;
                //Vector3.Lerp(rb.position, targetPosition, Time.fixedDeltaTime * 5f)
                rb.MovePosition(rb.position + direction * Time.fixedDeltaTime * 0.5f);
            }
            else
            {
    ...
    {
        if (PhotonNetwork.IsMasterClient && !collision.collider.CompareTag("tBall"))//PhotonNetwork.IsMasterClient &&
        {

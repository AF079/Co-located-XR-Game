...
public class NetworkedSphere : MonoBehaviour, IPunOwnershipCallbacks, IPunObservable
{
    ...
    void FixedUpdate()
    {
        if (!photonView.IsMine) //&& //!PhotonNetwork.IsMasterClient && !firstTake
        {
        ...
        if (PhotonNetwork.IsMasterClient && !photonView.IsMine)
        {
            ...
            if (stream.IsWriting)// && prevSentPos != transform.position
            {
                if (PhotonNetwork.IsMasterClient && photonView.IsMine)
                {
                    ...
                    Vector3 rotatedPosition = NetworkManager_test.MatMul(transformationMatrix_AtoB, localPosition);

                    stream.SendNext(rotatedPosition);
                    stream.SendNext(NetworkManager_test.MatMul(transformationMatrix_AtoB, rb.velocity - NetworkManager_test.originBinA));
                }else if(!PhotonNetwork.IsMasterClient && photonView.IsMine){

                    Vector3 localPosition = rb.position - NetworkManager_test.MatMul(transformationMatrix_BtoA,NetworkManager_test.originBinA); //Vector3.zero??

                    Vector3 rotatedPosition = NetworkManager_test.MatMul(transformationMatrix_BtoA, localPosition);

                    stream.SendNext(rotatedPosition);
                    stream.SendNext(NetworkManager_test.MatMul(transformationMatrix_BtoA, rb.velocity -
                        NetworkManager_test.MatMul(transformationMatrix_BtoA, NetworkManager_test.originBinA)));
            }
            }

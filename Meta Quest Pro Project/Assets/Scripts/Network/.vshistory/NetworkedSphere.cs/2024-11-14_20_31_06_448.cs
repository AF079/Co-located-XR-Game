...
public class NetworkedSphere : MonoBehaviour, IPunOwnershipCallbacks, IPunObservable
{
    ...
    public float pushForceMutliplier = 0.0005f;
    ...
    void Update()
    {
        ...
        handTimeStamp = PhotonNetwork.Time;
        prevLeftHandPos = LeftHand.transform.position;
        prevRightHandPos = RightHand.transform.position;
    }
    ...
    {
        if ((LeftHand != null && other.gameObject == LeftHand) || (RightHand != null && other.gameObject == RightHand) ||
            other.name.Contains("LeftController") || other.name.Contains("RightController"))
        {
            Debug.Log("Sphere touched by hand or controller!");
            isTouching = true;
    ...
    private void OnTriggerStay(Collider other)
    {
        if (isTouching && !other.CompareTag("tBall"))
        {
            if (photonView != null)
            {
                ...
                if (userPressTimestamps.Count <= 1 && LeftHand != null && RightHand != null)
                {

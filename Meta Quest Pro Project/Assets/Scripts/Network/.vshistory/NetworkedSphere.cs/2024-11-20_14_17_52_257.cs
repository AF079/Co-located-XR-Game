    ...
    void FixedUpdate()
    {
        ...
        if (PhotonNetwork.IsMasterClient)
        {
            ...
            if (distance > 0.01f) // Ignore small movements
            {
                ...
                rb.position = Vector3.MoveTowards(rb.position, simulatedPosition, Time.deltaTime * 1.5f);
            }
            else
            {
        ...
        else
        {
            ...
            if (distance > 0.01f)
            {
                rb.position = Vector3.MoveTowards(rb.position, targetPosition, Time.deltaTime * 1.5f);
            }
            else
            {
                rb.velocity = Vector3.zero;

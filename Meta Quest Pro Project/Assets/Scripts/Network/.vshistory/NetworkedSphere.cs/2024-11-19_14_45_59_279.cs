...
public class NetworkedSphere : MonoBehaviour, IPunOwnershipCallbacks, IPunObservable
{
    ...
    private float latencyCap = 0.07f;
    private float curMillis = 0.07f;
    private bool doAddLatency = false;
    ...
    void Start()
    {
        ...
        PhotonNetwork.NetworkingClient.LoadBalancingPeer.NetworkSimulationSettings.IncomingLag = 100; // 100ms delay for incoming messages
        PhotonNetwork.NetworkingClient.LoadBalancingPeer.NetworkSimulationSettings.OutgoingLag = 100; // 100ms delay for outgoing messages
        PhotonNetwork.NetworkingClient.LoadBalancingPeer.NetworkSimulationSettings.IncomingJitter = 20; // 20ms jitter
        PhotonNetwork.NetworkingClient.LoadBalancingPeer.NetworkSimulationSettings.OutgoingJitter = 20; // 20ms jitter
        PhotonNetwork.NetworkingClient.LoadBalancingPeer.NetworkSimulationSettings.IncomingLossPercentage = 5; // 5% packet loss
        PhotonNetwork.NetworkingClient.LoadBalancingPeer.NetworkSimulationSettings.OutgoingLossPercentage = 5; // 5% packet loss
    }
    ...
    bool addLatency()
    {
        ...
        else if (doAddLatency && curMillis < latencyCap)
        {
            curMillis += Time.deltaTime;
        }
        return doAddLatency;

...
public class NetworkedSphere : MonoBehaviour, IPunOwnershipCallbacks, IPunObservable
{
    ...
    private struct TransformUpdate
    {
        ...
        public float deliveryTime; // When this update should be applied
    }

    private Queue<TransformUpdate> latencyQueue = new Queue<TransformUpdate>();
    public float simulatedLatency = 0.1f; // 100ms latency
    public float simulatedJitter = 0.02f; // 20ms jitter
    public float packetLossPercentage = 5f; // 5% packet loss
    ...
    void FixedUpdate()
    {
        ...
        if (PhotonNetwork.IsMasterClient)
        {
            SimulateLatencyForMaster();
        }
    }

    void SimulateLatencyForMaster()
    {
        // Enqueue the current transform update with simulated delay
        if(UnityEngine.Random.Range(0,1) > 0.5f) { 
            if (UnityEngine.Random.Range(0f, 100f) >= packetLossPercentage) // Simulate packet loss
            {
                float jitter = UnityEngine.Random.Range(-simulatedJitter, simulatedJitter); // Simulate jitter
                latencyQueue.Enqueue(new TransformUpdate
                {
                    position = rb.position,
                    velocity = rb.velocity,
                    deliveryTime = Time.time + simulatedLatency + jitter
                });
            }
        }

        // Apply updates that have "arrived" (based on deliveryTime)
        while (latencyQueue.Count > 0 && latencyQueue.Peek().deliveryTime <= Time.time)
        {
            TransformUpdate update = latencyQueue.Dequeue();
            rb.position = update.position;

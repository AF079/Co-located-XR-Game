...
public class NetworkedSphere : MonoBehaviour, IPunOwnershipCallbacks, IPunObservable
{
    ...
    void FixedUpdate()
    {
        ...
        if (PhotonNetwork.IsMasterClient)
        {
            ...
            rb.MovePosition(Vector3.Lerp(rb.position, simulatedPosition, Time.fixedDeltaTime * 8f));
        ...
        else
        {
            ...
            {
                rb.MovePosition(Vector3.Lerp(rb.position, targetPosition, Time.fixedDeltaTime * 8f));
            }
            else
            {

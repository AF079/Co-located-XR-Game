...
public class NetworkedSphere : MonoBehaviour, IPunOwnershipCallbacks, IPunObservable
{
    ...
    private float jitterIntensity = 0.08f;
    private Vector3 simulatedPosition;
    private float baseLatency = 0.25f;
    ...
    private Vector3 masterVelocity; // For master client
    private Vector3 nonMasterVelocity; // For non-master client
    ...
    void FixedUpdate()
    {
        ...
        if (PhotonNetwork.IsMasterClient)
        {
            //transform.position = Vector3.Lerp(transform.position, simulatedPosition, Time.deltaTime);
            //rb.position = Vector3.SmoothDamp(rb.position, simulatedPosition, ref masterVelocity, 0.3f);
            rb.MovePosition(Vector3.SmoothDamp(rb.position, simulatedPosition, ref masterVelocity,0.3f));

        }
        else
        {
            //transform.position = Vector3.Lerp(transform.position, targetPosition, Time.deltaTime);
            //rb.position = Vector3.SmoothDamp(rb.position, targetPosition, ref nonMasterVelocity, 0.3f);
            rb.MovePosition(Vector3.SmoothDamp(rb.position, targetPosition, ref nonMasterVelocity, 0.3f));

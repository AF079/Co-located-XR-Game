...
public class NetworkedSphere : MonoBehaviour, IPunOwnershipCallbacks, IPunObservable
{
    ...
    void Update()
    {
        ...
        if (PhotonNetwork.IsMasterClient)
        {
            ...
            transform.position = Vector3.SmoothDamp(rb.position, simulatedPosition, ref velocity, 0.3f);

        }
        else
        {
            //transform.position = Vector3.Lerp(transform.position, targetPosition, Time.deltaTime);
            transform.position = Vector3.SmoothDamp(rb.position, targetPosition, ref velocity, 0.3f);
        }

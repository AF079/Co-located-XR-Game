...
public class JoinGameManager : MonoBehaviourPunCallbacks
{
    ...
    private void OnLoggedInUser(Message<User> msg)
    {
        ...
        // Retrieve the user's Oculus ID (a ulong)
        ulong userID = msg.Data.ID;
        Debug.Log("Logged in User ID: " + userID);

        // Now that you have the userID, you can use it to share spatial anchors
        // Create an OVRSpaceUser with this ID
        OVRSpaceUser spaceUser = new OVRSpaceUser(userID);
    }

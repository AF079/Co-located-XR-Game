...
public class ClientManager : MonoBehaviourPunCallbacks
{
    ...
    private void moveSpheres()
    {
        ...
        origin.transform.position = new Vector3(0,0,0); // move(origin.transform);
        up.transform.position = new Vector3(0, 0, 0); //move(up.transform);
        side.transform.position = new Vector3(0, 0, 0); //move(side.transform);
        forward.transform.position = new Vector3(0, 0, 0); //move(forward.transform);
        someCube.transform.position = new Vector3(0, 0, 0);// move(someCube.transform);
    ...
    void Update()
    {
        if (!PhotonNetwork.IsMasterClient) {
            ...
            if (!hasRepositioned && receivedData && deactivated) //&& regP1 && regP2 
            {
                ...
                /*PhotonView masterClientPhotonView = PhotonView.Find(PhotonNetwork.MasterClient.ActorNumber);
                if (masterClientPhotonView != null)
                {
                    masterClientPhotonView.RPC("receiveSetActiveSignal", RpcTarget.MasterClient);
                }*/
